...
static esp_err_t ota_api_get_handler(httpd_req_t *req) {
    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK) {
        ...
        if (httpd_query_key_value(query, "backup", backup_type, sizeof(backup_type)) == ESP_OK) {
            ...
            // Set headers to force download on all browsers including iOS Safari
            httpd_resp_set_type(req, "application/force-download");
            ...
            // Additional headers for iOS Safari and other stubborn browsers
            httpd_resp_set_hdr(req, "Content-Description", "File Transfer");
