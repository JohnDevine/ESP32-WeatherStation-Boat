...
static esp_err_t ota_api_get_handler(httpd_req_t *req) {
    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK) {
        ...
        if (httpd_query_key_value(query, "backup", backup_type, sizeof(backup_type)) == ESP_OK) {
            ...
            // Check if this is a captive portal request (detect iOS Safari and other captive browsers)
            bool is_captive_browser = false;
            char user_agent[256] = "";
            if (httpd_req_get_hdr_value_str(req, "User-Agent", user_agent, sizeof(user_agent)) == ESP_OK) {
                ...
                if (strstr(user_agent, "CaptiveNetworkSupport") || 
                    strstr(user_agent, "Safari") ||
                    strstr(user_agent, "Mobile")) {
                    is_captive_browser = true;
                    ESP_LOGI(TAG, "Captive portal browser detected: %s", user_agent);
                }
            }

            if (is_captive_browser) {
                // For captive browsers, send an HTML page with base64 data and download link
                httpd_resp_set_type(req, "text/html");
                
                // Send HTML header with embedded JavaScript download
                const char* html_start = 
                    "<!DOCTYPE html>"
                    "<html><head><title>Firmware Download</title>"
                    "<meta charset='UTF-8'>"
                    "<meta name='viewport' content='width=device-width, initial-scale=1.0'>"
                    "<style>"
                    "body { font-family: Arial, sans-serif; margin: 20px; text-align: center; }"
                    ".download-btn { background: #007AFF; color: white; border: none; padding: 15px 30px; border-radius: 8px; font-size: 16px; cursor: pointer; }"
                    ".info { background: #f0f0f0; padding: 15px; margin: 20px 0; border-radius: 8px; }"
                    "</style>"
                    "</head><body>"
                    "<h2>Firmware Backup Download</h2>"
                    "<div class='info'>"
                    "<p><strong>File:</strong> esp32_%s_backup.bin</p>"
                    "<p><strong>Size:</strong> %d bytes</p>"
                    "<p>Click below to download your firmware backup:</p>"
                    "</div>";

                char html_header[800];
                snprintf(html_header, sizeof(html_header), html_start, backup_type, part->size);
                httpd_resp_sendstr_chunk(req, html_header);

                // Read and encode partition in base64 chunks
                const size_t buf_size = 1023; // Multiple of 3 for clean base64 encoding
                uint8_t *buf = malloc(buf_size);
                char *b64_buf = malloc((buf_size * 4 / 3) + 4); // Base64 is ~4/3 larger
                ...
                // Start JavaScript with base64 data
                httpd_resp_sendstr_chunk(req, "<script>var binaryData = '");
                ...
                while (offset < part->size) {
                    ...
                    // Encode to base64
                    size_t b64_len = 0;
                    mbedtls_base64_encode((unsigned char*)b64_buf, (buf_size * 4 / 3) + 4, &b64_len, buf, to_read);
                    b64_buf[b64_len] = '\0';
                    
                    // Send base64 chunk
                    if (httpd_resp_sendstr_chunk(req, b64_buf) != ESP_OK) {
                        free(buf);
                        free(b64_buf);
                        return ESP_FAIL;
                    }
                ...
                // Complete JavaScript and HTML (split into multiple chunks to avoid buffer overflow)
                httpd_resp_sendstr_chunk(req, "';</script>");
                
                // Send the download button and script separately
                const char* download_script = 
                    "<script>"
                    "function downloadFile() {"
                        "try {"
                            "var byteChars = atob(binaryData);"
                ...
                httpd_resp_sendstr_chunk(req, download_script);
                
                // Send filename part
                char filename_script[200];
                snprintf(filename_script, sizeof(filename_script),
                    "link.download = 'esp32_%s_backup.bin';", backup_type);
                httpd_resp_sendstr_chunk(req, filename_script);
                
                // Send remaining JavaScript and HTML
                const char* final_html = 
                    ...
                    "}"
                    "</script>"
                    "<button class='download-btn' onclick='downloadFile()'>Download Firmware Backup</button>"
                    "<p><small>If download doesn't work, try opening this page in a desktop browser.</small></p>"
                    "</body></html>";

                httpd_resp_sendstr_chunk(req, final_html);
                httpd_resp_sendstr_chunk(req, NULL); // End response

                free(buf);
                free(b64_buf);
            } else {
                // For regular browsers, use standard binary download
...
static esp_err_t ota_api_post_handler(httpd_req_t *req) {
    ...
    char* skptr = strstr(header_buf, "name=\"skipBackup\"");
    if (skptr && skptr < header_buf + header_read) {
        char *val = strstr(skptr, "\r\n\r\n");
        if (val && val < header_buf + header_read) {
            val += 4;
            if (val < header_buf + header_read && strncmp(val, "1", 1) == 0) {
