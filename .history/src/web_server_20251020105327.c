...
static esp_err_t ota_api_post_handler(httpd_req_t *req) {
    ...
    // Use a small buffer for streaming processing
    ...
    // Read the first chunk to parse headers
    int received = 0;
    int r = httpd_req_recv(req, buf, CHUNK_SIZE - 1);
    if (r <= 0) {
        free(buf);
    ...
    if (skptr && skptr < buf + r) {
        char *val = strstr(skptr, "\r\n\r\n");
        if (val && val < buf + r) {
            val += 4;
            if (val < buf + r && strncmp(val, "1", 1) == 0) cfg.create_backup = false;
        }
    }
    ...
    // Calculate how much file data is in the first chunk
    int header_size = data_start - buf;
    int first_data_size = r - header_size;
    ...
    if (first_data_size > 0) {
        ret = ota_process_chunk((const uint8_t*)data_start, first_data_size);
        if (ret != ESP_OK) {
            ota_auto_rollback();
            free(buf);
    ...
    while (received < total) {
        int to_recv = (total - received > CHUNK_SIZE) ? CHUNK_SIZE : (total - received);
        r = httpd_req_recv(req, buf, to_recv);
        if (r <= 0) {
            ota_auto_rollback();
            free(buf);
            httpd_resp_send_err(req, HTTPD_408_REQ_TIMEOUT, "Receive error");
        ...
        // Process this chunk, but skip the boundary at the end
        int process_size = r;
        if (received >= total) {
            // Last chunk - look for boundary
            char *boundary = strstr(buf, "\r\n--");
            if (boundary) {
                process_size = boundary - buf;
            }
        }

        if (process_size > 0) {
            ret = ota_process_chunk((const uint8_t*)buf, process_size);
            if (ret != ESP_OK) {
                ota_auto_rollback();
                free(buf);
                httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "OTA write failed");
