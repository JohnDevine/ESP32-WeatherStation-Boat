...
esp_err_t nvs_store_boot_count(uint32_t count) {
    // Use SystemMetrics NVS handle directly by opening the metrics namespace
    nvs_handle_t nvs_handle;
    esp_err_t err = nvs_open("metrics", NVS_READWRITE, &nvs_handle);
    if (err != ESP_OK) {
        // If metrics namespace doesn't exist, create it
        if (err == ESP_ERR_NVS_NOT_FOUND) {
            ESP_LOGW(TAG, "Metrics namespace not found, creating it");
            err = nvs_open("metrics", NVS_READWRITE, &nvs_handle);
        }
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Failed to open metrics NVS handle: %s", esp_err_to_name(err));
            return err;
        }
    }
    
    // Use the same key and method as SystemMetrics
    err = nvs_set_blob(nvs_handle, "boot_count", &count, sizeof(count));
    if (err == ESP_OK) {
        err = nvs_commit(nvs_handle);
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "Stored boot count: %lu", count);
        } else {
            ESP_LOGE(TAG, "Failed to commit boot count to NVS: %s", esp_err_to_name(err));
        }
    } else {
        ESP_LOGE(TAG, "Failed to set boot count in NVS: %s", esp_err_to_name(err));
    }
...
esp_err_t nvs_load_boot_count(uint32_t *count) {
    ...
    // Try to get the boot count from SystemMetrics API first
    const char* boot_count_str = get_system_metric(METRIC_BOOT_COUNT);
    if (boot_count_str && strstr(boot_count_str, "ERROR") == NULL) {
        // Parse the boot count from the string (format: "123 boots")
        *count = (uint32_t)atol(boot_count_str);
        ESP_LOGI(TAG, "Loaded boot count from SystemMetrics: %lu", *count);
        return ESP_OK;
    }
    
    // Fallback to direct NVS access if SystemMetrics fails
    nvs_handle_t nvs_handle;
    esp_err_t err = nvs_open("metrics", NVS_READONLY, &nvs_handle);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "Failed to open metrics NVS handle: %s", esp_err_to_name(err));
        *count = 0;
        return ESP_OK; // Return success with default value
    }
    
    size_t required_size = sizeof(*count);
    err = nvs_get_blob(nvs_handle, "boot_count", count, &required_size);
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Loaded boot count from NVS: %lu", *count);
    } else if (err == ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGW(TAG, "Boot count not found in NVS, using default (0)");
        *count = 0;
        err = ESP_OK;
    } else {
        ESP_LOGE(TAG, "Failed to get boot count from NVS: %s", esp_err_to_name(err));
        *count = 0;
        err = ESP_OK; // Return success with default value
    }
