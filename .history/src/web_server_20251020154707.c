    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK) {
        ...
        if (httpd_query_key_value(query, "backup", backup_type, sizeof(backup_type)) == ESP_OK) {
            ...
            if (is_captive_browser) {
                // For captive browsers, use a simpler approach to avoid memory issues
                // Large firmware files (900KB+) cause crashes when base64 encoded
                httpd_resp_set_type(req, "text/html");
                
                const char* error_page = 
                    "<!DOCTYPE html>"
                    "<html><head><title>Download Not Available</title>"
                    "<meta charset='UTF-8'>"
                    ...
                    ".btn { background: #007AFF; color: white; border: none; padding: 15px 30px; border-radius: 8px; font-size: 16px; text-decoration: none; display: inline-block; }"
                    "</style>"
                    "</head><body>"
                    "<h2>Firmware Backup</h2>"
                    "<div class='warning'>"
                    "<h3>⚠️ Captive Portal Limitation</h3>"
                    "<p>Firmware downloads are not available in captive portal mode due to memory limitations.</p>"
                    "</div>"
                    "<div class='info'>"
                    ...
                    "<li>Connect to ESP32WebServer from a <strong>desktop browser</strong></li>"
                    "<li>Use the <strong>regular browser</strong> (not captive portal)</li>"
                    "<li>Access via <strong>http://192.168.4.1/api/ota?backup=%s</strong></li>"
                    "</ul>"
                    "</div>"
                    "<a href='/' class='btn'>Return to Home</a>"
                    "<p><small>This limitation prevents system crashes when handling large firmware files.</small></p>"
                    "</body></html>";

                char error_html[2000];
                snprintf(error_html, sizeof(error_html), error_page, backup_type, part->size, backup_type);
                httpd_resp_sendstr_chunk(req, error_html);
                httpd_resp_sendstr_chunk(req, NULL); // End response
            } else {
                // For regular browsers, also show the information page to prevent crashes
                // Binary downloads are currently disabled due to memory issues
                httpd_resp_set_type(req, "text/html");
                
                const char* regular_browser_page = 
                    "<!DOCTYPE html>"
                    "<html><head><title>Download Temporarily Disabled</title>"
                    "<meta charset='UTF-8'>"
                    "<meta name='viewport' content='width=device-width, initial-scale=1.0'>"
                    "<style>"
                    "body { font-family: Arial, sans-serif; margin: 20px; text-align: center; }"
                    ...
                    "<h2>Firmware Backup</h2>"
                    "<div class='warning'>"
                    "<h3>⚠️ Download Temporarily Disabled</h3>"
                    "<p>Firmware downloads are temporarily disabled to prevent system crashes.</p>"
                    "</div>"
                    "<div class='info'>"
                    "<p><strong>File:</strong> esp32_%s_backup.bin</p>"
                    "<p><strong>Size:</strong> %d bytes</p>"
                    "<p>This feature will be re-enabled once the memory issue is resolved.</p>"
                    "</div>"
                    "<a href='/' class='btn'>Return to Home</a>"
                    "</body></html>";

                char regular_html[1500];
                snprintf(regular_html, sizeof(regular_html), regular_browser_page, backup_type, part->size);
                httpd_resp_sendstr_chunk(req, regular_html);
                httpd_resp_sendstr_chunk(req, NULL); // End response
            }
            return ESP_OK;
        }
