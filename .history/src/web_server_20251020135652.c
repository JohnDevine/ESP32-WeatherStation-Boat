...
static void reboot_task(void *pvParameter) {
    ESP_LOGI(TAG, "Reboot task starting - waiting 3 seconds");
...
static esp_err_t ota_api_post_handler(httpd_req_t *req) {
    ...
    if (ret == ESP_OK) {
        ...
        if (status.reboot_required) {
            // Filesystem update - notify client and schedule reboot
            httpd_resp_send(req, "{\"status\":\"ok\",\"reboot\":true,\"message\":\"Filesystem updated, rebooting in 3 seconds\"}", -1);
            ESP_LOGI(TAG, "OTA filesystem update completed - scheduling reboot in 3 seconds");
            
            // Schedule reboot using a timer task to avoid blocking HTTP response
            static TimerHandle_t reboot_timer = NULL;
            if (reboot_timer != NULL) {
                xTimerDelete(reboot_timer, 0);
            }
            reboot_timer = xTimerCreate("reboot_timer", pdMS_TO_TICKS(3000), pdFALSE, NULL, 
                                       [](TimerHandle_t timer) { 
                                           ESP_LOGI("WEB_SERVER", "Rebooting system now...");
                                           esp_restart(); 
                                       });
            if (reboot_timer != NULL) {
            ...
            // Firmware update - just notify success (firmware updates reboot automatically)
            httpd_resp_send(req, "{\"status\":\"ok\",\"message\":\"Firmware updated successfully\"}", -1);
