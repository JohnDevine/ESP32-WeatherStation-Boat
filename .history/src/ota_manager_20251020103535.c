...
esp_err_t ota_start_update(const ota_config_t* config) {
    ...
    if (config->verify_crypto && config->expected_hash[0] != '\0') {
        ...
        mbedtls_sha256_starts(&g_sha256_ctx, 0); // SHA256, not SHA224
        g_crypto_init = true;
    }
...
esp_err_t ota_process_chunk(const uint8_t* data, size_t size) {
    ...
    if (g_crypto_init) {
        mbedtls_sha256_update_ret(&g_sha256_ctx, data, size);
    }
...
esp_err_t ota_finalize_update(void) {
    ...
    if (g_crypto_init && g_ota_config.expected_hash[0] != '\0') {
        ...
        mbedtls_sha256_finish_ret(&g_sha256_ctx, computed_hash);
    ...
    if (g_ota_status.type == OTA_TYPE_FIRMWARE) {
        ...
        system_metrics_update_boot_count(0); // Reset boot count for new firmware
        
    } else {
        ESP_LOGI(TAG, "Filesystem update completed");
    }
